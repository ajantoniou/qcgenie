...
const PYTHON = process.env.UPLOADCHECK_PYTHON || "python3";
const YTDLP = process.env.UPLOADCHECK_YTDLP || "yt-dlp";
...
export function runQcEngine(videoPath, opts = {}) {
  ...
  if (opts.expectedScriptPath) args.push("--expected-script", opts.expectedScriptPath);
  if (opts.sidecarDir) args.push("--sidecar-dir", opts.sidecarDir);
