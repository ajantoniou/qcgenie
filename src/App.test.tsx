...
describe("UploadCheck conversion UI", () => {
  ...
  test("positions the metered pricing model around the $99 creator plan", () => {
    ...
    expect(screen.getByText(/2,400 checked minutes\/month/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/Included minutes cover deterministic publish-readiness QC/)).toBeInTheDocument();
    expect(screen.getByText(/Deep model review stays internal/)).toBeInTheDocument();
    expect(screen.getByText(/deterministic capture rate against expert review/)).toBeInTheDocument();
