...
2. Run public npm MCP install through Claude Code, Codex, Cursor, and local NTO production using the `uploadcheck` MCP server.
   - Install handoff: `docs/PRIVATE-MCP-BETA.md`
3. Keep registry proof current for `@drantoniou/uploadcheck` and `@drantoniou/uploadcheck-mcp`.
