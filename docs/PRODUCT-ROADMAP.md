...
- Done: live readiness now exposes Render media-ingress proof for inline ephemeral `video_base64`, `audio_base64`, `media_base64`, and `data_url` jobs plus signed-upload fallback, so agents can verify programmatic media submission before relying on the hosted API.
