...
- Partial: public download/listing is still not ready because npm publish, hosted `/mcp-install.json` redeploy proof, hosted launch-doctor/evidence freshness, and external Claude Code/Codex/Cursor beta evidence remain outstanding.
- Next: redeploy the current API/static artifacts to Render, then run `npm run live-mcp-install:verify`, `npm run live-public-artifacts:verify`, `npm run live-launch-doctor:verify`, and `npm run live-launch-evidence:verify`.
