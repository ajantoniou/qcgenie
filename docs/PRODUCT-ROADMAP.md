...
- Done: `npm run media-ingress:verify` now smoke-tests inline ephemeral `video_base64` and `audio_base64` jobs plus signed-upload fallback, and live readiness exposes the same Render media-ingress modes for agents before they rely on the hosted API.
