...
- Done: public npm packages `@drantoniou/uploadcheck` and `@drantoniou/uploadcheck-mcp` are published with registry install proof. External Claude Code/Codex/Cursor public npm MCP beta evidence, Hosted MCP install, cost basis, web artifacts, launch doctor/evidence, and public artifacts now pass.
- Done: npm-based MCP clients are verified against the public install artifact and must use a created UploadCheck API key tied to included plan minutes or an operator-created account.
- Done: billing copy now states included minutes reset monthly and do not roll over. Users upgrade plans or contact the operator for approved extra-minute capacity until a self-serve add-on flow exists.
- Next: prepare an Anthropic Directory submission around the narrow MCP/API workflow after hosted paid-workspace proof and spend-alert proof are bundled. Defer OpenAI ChatGPT app/connector work until a hosted HTTPS MCP endpoint, account binding, and ChatGPT-native report UX are worth the review effort.
...
- Done: Product Hunt launch page, public report examples, bundled demo clip, custom domains, checkout/webhook, mounted persistence/storage envs, hosted secret encryption, npm packages, registry install proof, and Claude Code/Codex/Cursor beta proof are live-ready.
