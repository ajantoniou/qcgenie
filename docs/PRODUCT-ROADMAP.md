...
Pricing verdict: `$99 / 5,000 minutes` is too generous if it implies unlimited full-video AI review. Customer pricing should sell deterministic publish-readiness QC minutes, not AI review minutes. At `0.0833` COGS cents/minute for deterministic scanning, launch pricing can safely double the included minutes to `Creator $99 / 2,400`, `Studio $299 / 10,000`, and `Network $899 / 36,000` while staying above the 95% gross-margin target. Full-model review stays internal for backtesting, roadmap generation, and deterministic capture-rate measurement.

Cost-per-minute target: at `$99 / 2,400`, the deterministic-only full-allowance COGS is about `$1.9992`, or about `97.98%` gross margin. At `$299 / 10,000`, deterministic full-allowance COGS is about `$8.33`, or about `97.21%` gross margin. At `$899 / 36,000`, deterministic full-allowance COGS is about `$29.988`, or about `96.66%` gross margin. Full Gemini/Qwen/hosted transcription still stays out of the customer meter and is used internally for backtesting, roadmap generation, and deterministic capture-rate measurement.
...
- Done: Render launch env now requires `UPLOADCHECK_LEMONSQUEEZY_WEBHOOK_SECRET` and preserves `api_keys:write/api_keys:read` in `UPLOADCHECK_API_SCOPES`, so paid checkout provisioning and dashboard-created MCP keys work after deploy.
...
- Done: stored workspace API keys now force their server-side workspace, owner, plan, included-minute, and subscription-price metadata over client-supplied fields. Stored-key upload/webhook/job/report/event/artifact/marker/gate-verdict/list/usage access is limited to the key's own workspace, queued job drains only process the key's own workspace jobs, active-job concurrency is evaluated within that workspace, and stored-key API-key review/provisioning routes are pinned to the key's workspace and owner even if a client supplies another workspace.
- Done: approved overage credits are now enforced through `overage_cap_cents` on workspace API keys. A zero or omitted cap blocks at included deterministic minutes, while a positive cap allows extra deterministic minutes only until projected billable extra-minute spend reaches that cap; delegated stored-key creation inherits the parent key's plan economics and cap.
...
- Done: direct `run_gate.py` defaults are now deterministic/customer-safe; paid oracle checks such as `twins`, `omni_watch`, `gemini_watch`, `narration_match`, `cheap_broll`, and `garble` require explicit `--checks`, and regression coverage prevents `--fast` from being mistaken for a deterministic-only guardrail.
- Done: abuse limits added for file size, declared duration, and active job concurrency. Job/upload requests now fail fast with `duration_limit_exceeded`, `upload_size_limit_exceeded`, or `active_job_limit_exceeded` before QC compute or storage spend, and persisted abuse events are exposed through the dashboard plus authenticated `GET /v1/abuse-events` for operator review.
...
- Done: checkout/account provisioning endpoint added at `POST /v1/checkout/provision-api-key`; it creates idempotent paid workspace API keys from plan/customer/subscription metadata, applies UploadCheck plan economics server-side, and returns the bearer only on first provisioning.
...
- Done: private MCP beta handoff added at `docs/PRIVATE-MCP-BETA.md`, and the machine-readable MCP install manifest now includes the `UPLOADCHECK_API_KEY` placeholder for Codex private-beta installs.
- Done: private MCP beta verifier added through `npm run private-mcp-beta:verify`, checking the handoff, MCP install manifest, package scripts, Directory prep, workspace-key rule, and no-public-self-serve claims.
- Done: private MCP beta evidence contract added at `docs/private-mcp-beta-evidence-template.json` with `npm run private-mcp-beta:evidence`, so Claude Code, Codex, and Cursor proof must be captured as sanitized job/report evidence before public download or Directory submission.
...
- Done: product-agent readiness verifier added through `npm run product-agent:verify`, giving the current external-agent verdict: private MCP beta is usable with workspace API keys, but public npm/download and Directory/connector distribution remain blocked.
...
- Done: MCP install manifest now separates current private-beta local/private-clone snippets from future `npx -y @uploadcheck/mcp` snippets, and verifiers require credit-gated workspace API-key language before external Claude/Codex/Cursor handoff.
- Done: Anthropic Directory verifier now requires `npm run saas-basics:verify`, credit-gated workspace distribution, private-beta MCP install status, and the SaaS submission blocker before Directory prep can pass.
...
- Done: live launch-doctor/evidence verifiers now require SaaS, MCP install, private MCP beta, and Anthropic Directory proof commands in hosted command coverage, so stale Render launch endpoints cannot pass after local launch gates change.
- Done: deployment cutover and publish checklist now name the stale hosted proof explicitly: redeploy must refresh `/mcp-install.json`, launch status/Product Hunt kit, and hosted launch doctor/evidence command coverage before live launch checks can pass.
...
- Partial: public download/listing is still not ready because npm publish, hosted launch-doctor/evidence/public-artifact command coverage for `private-mcp-beta:evidence`, and external Claude Code/Codex/Cursor beta evidence remain outstanding. Hosted MCP install, cost basis, and web artifacts now pass.
- Next: redeploy the current API/static artifacts to Render, then run `npm run live-public-artifacts:verify`, `npm run live-launch-doctor:verify`, and `npm run live-launch-evidence:verify` to prove `private-mcp-beta:evidence` is included in hosted command coverage.
- Next: publish `@uploadcheck/mcp` and `@uploadcheck/cli`, capture registry install proof, and verify external Claude Code, Codex, Cursor, and MCP clients can call deterministic QC only with a created UploadCheck API key tied to subscription minutes or top-up credits.
- Next: after package publish, hosted MCP install proof, and private MCP beta evidence are clean, prepare an Anthropic Directory submission around the narrow MCP/API workflow. Defer OpenAI ChatGPT app/connector work until a hosted HTTPS MCP endpoint, account binding, and ChatGPT-native report UX are worth the review effort.
