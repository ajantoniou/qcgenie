...
That command checks the public package names, CLI/MCP bins, MCP lock metadata, and `npm pack --dry-run` file lists for `@drantoniou/uploadcheck` and `@drantoniou/uploadcheck-mcp`.
...
For NTO final masters, run deterministic UploadCheck first, then run the internal Gemini oracle from the private repo with `python3 scripts/qc-engine/gemini_watch.py <file> --transcript <transcript-or-words.json> --json <gemini-watch.json>`. Use the Gemini output only to measure deterministic capture rate and identify roadmap misses. Do not expose Gemini through the public MCP package, present Gemini as the customer verdict, or count it as included-minute review.
...
- Model-backed checks such as `twins`, `cheap_broll`, `garble`, `narration_match`, and `omni_watch` are also counted in the guardrail. If the caller omits `checks`, the engine default is deterministic/customer-safe and excludes paid oracle checks. Paid model-backed checks run only when explicitly requested and then pass through the `downgrade`, `block`, or `off` guardrail. When `twins` is explicitly requested and a manifest is supplied, it first uses manifest-marked duplicate, near-duplicate, or almost-identical character evidence before any vision call.
