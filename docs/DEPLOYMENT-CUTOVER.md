...
The launch doctor also includes `npm run live-mcp-install:verify` and reports `hosted-mcp-install` as a blocker until hosted `/mcp-install.json` exposes public npm MCP install status, GitHub/local fallback snippets, and workspace API-key placeholders.
