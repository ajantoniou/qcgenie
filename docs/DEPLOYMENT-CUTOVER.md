...
- Checkout prompts: HTTPS direct `UPLOADCHECK_CREATOR_CHECKOUT_URL`, `UPLOADCHECK_STUDIO_CHECKOUT_URL`, `UPLOADCHECK_NETWORK_CHECKOUT_URL`, or Lemon Squeezy `UPLOADCHECK_LEMONSQUEEZY_STORE_SLUG` / `UPLOADCHECK_LEMONSQUEEZY_STORE_URL` plus `UPLOADCHECK_CREATOR_VARIANT_ID`, `UPLOADCHECK_STUDIO_VARIANT_ID`, and `UPLOADCHECK_NETWORK_VARIANT_ID`; set `UPLOADCHECK_LEMONSQUEEZY_WEBHOOK_SECRET` so signed checkout events can provision MCP/API keys.
...
The hosted launch doctor/evidence verifiers also require `saas-basics:verify`, `mcp-install:verify`, `private-mcp-beta:verify`, `private-mcp-beta:evidence`, `anthropic-directory:verify`, and `product-agent:verify` in hosted command coverage; after local launch gates change, redeploy Render before treating those live checks as current.
...
The launch doctor also includes `npm run live-mcp-install:verify` and reports `hosted-mcp-install` as a blocker until hosted `/mcp-install.json` exposes public GitHub/local install status, current local snippets, future npm guardrails, and workspace API-key placeholders.
The launch doctor also includes `npm run live-public-artifacts:verify` and reports `hosted-public-artifacts` as a blocker until hosted `/launch-status.json`, `/product-hunt-launch-kit.json`, `/mcp-install.json`, `/sample-reports/index.json`, the individual PASS/WATCH/BLOCK sample reports, and `/llms.txt` expose the current launch-evidence, cost, MCP install, sample-report, and public go/no-go contract.
If hosted `/mcp-install.json` returns 404 or omits the public GitHub/local install plus future-npm distinction, redeploy the API/static artifacts before handing MCP install instructions to external Claude, Codex, Cursor, or MCP clients.
...
`render:validate-env-file -- /tmp/uploadcheck-render-launch.env` checks the filled local file before it is sourced. `render:validate-env` checks the currently loaded shell environment before apply: real Render API key, valid API-key hash or bootstrap key, HTTPS checkout URLs or Lemon Squeezy store slug/store URL plus variant inputs, Lemon Squeezy webhook signing secret, strong webhook encryption key, durable `/mnt/...` paths, and complete optional object-storage settings. `launch:checkout` and `/v1/readiness` also reject non-HTTPS direct checkout URLs so Product Hunt readiness cannot pass with an insecure payment link. `render:apply` refuses to run when validation fails.

If Lemon Squeezy API access is available locally, run `npm run launch:checkout-discover` before filling checkout env. It looks for a published UploadCheck product with monthly Creator `$99`, Studio `$299`, and Network `$899` subscription variants, then prints redacted env names and next actions. After checkout env is configured, run `UPLOADCHECK_CHECKOUT_PROBE=1 npm run launch:checkout` in the same environment. The regular checkout helper validates env shape and HTTPS without network calls; probe mode performs redacted live HEAD/GET reachability checks for Creator, Studio, and Network checkout URLs. Output shows only host and redacted checkout URL shape, never direct checkout paths or Lemon Squeezy variant IDs.
