...
The hosted launch doctor/evidence verifiers also require `saas-basics:verify`, `mcp-install:verify`, `private-mcp-beta:verify`, `anthropic-directory:verify`, and `product-agent:verify` in hosted command coverage; after local launch gates change, redeploy Render before treating those live checks as current.
...
The launch doctor also includes `npm run live-mcp-install:verify` and reports `hosted-mcp-install` as a blocker until hosted `/mcp-install.json` exposes private-beta status, current local/private-clone snippets, future npm guardrails, and workspace API-key placeholders.
...
If hosted `/mcp-install.json` returns 404 or omits the private-beta local install plus future-npm distinction, redeploy the API/static artifacts before handing MCP install instructions to external Claude, Codex, Cursor, or MCP clients.
