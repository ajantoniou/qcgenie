...
- Checkout prompts: HTTPS direct `UPLOADCHECK_CREATOR_CHECKOUT_URL`, `UPLOADCHECK_STUDIO_CHECKOUT_URL`, `UPLOADCHECK_NETWORK_CHECKOUT_URL`, or Lemon Squeezy `UPLOADCHECK_LEMONSQUEEZY_STORE_SLUG` / `UPLOADCHECK_LEMONSQUEEZY_STORE_URL` plus `UPLOADCHECK_<PLAN>_VARIANT_ID`; set `UPLOADCHECK_LEMONSQUEEZY_WEBHOOK_SECRET` so signed checkout events can provision MCP/API keys.
