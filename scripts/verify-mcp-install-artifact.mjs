...
if (agentManifest.distribution?.status !== "private_mcp_beta_not_public_self_serve") {
  errors.push({ key: "agent-manifest.distribution.status", reason: "missing_private_beta_status" });
}
if (agentManifest.distribution?.current_install !== "local_checkout_or_private_clone") {
  errors.push({ key: "agent-manifest.distribution.current_install", reason: "missing_local_private_clone_install" });
}
if (agentManifest.distribution?.public_download_ready !== false) {
  errors.push({ key: "agent-manifest.distribution.public_download_ready", reason: "must_not_claim_public_download_ready" });
}
...
if (!String(agentManifest.distribution?.future_npm_install || "").includes("after @uploadcheck/mcp is published")) {
  errors.push({ key: "agent-manifest.distribution.future_npm_install", reason: "missing_future_npm_guard" });
}
...
if (!agentManifest.workspace_key_contract?.private_beta_required) {
  errors.push({ key: "agent-manifest.workspace_key_contract.private_beta_required", reason: "missing_workspace_key_private_beta_rule" });
}
if (!String(agentManifest.workspace_key_contract?.external_users_need || "").includes("workspace API key tied to plan minutes")) {
  errors.push({ key: "agent-manifest.workspace_key_contract.external_users_need", reason: "missing_credit_gated_workspace_key_rule" });
}
...
for (const [label, manifest] of [["package", packageManifest], ["public", publicManifest]]) {
  ...
  if (manifest.package !== "@uploadcheck/mcp") errors.push({ key: `${label}.package`, reason: "wrong_package" });
  if (manifest.binary !== "uploadcheck-mcp") errors.push({ key: `${label}.binary`, reason: "wrong_binary" });
  if (manifest.distribution_status !== "private_mcp_beta_not_public_self_serve") errors.push({ key: `${label}.distribution_status`, reason: "missing_private_beta_status" });
  if (manifest.current_install !== "local_checkout_or_private_clone") errors.push({ key: `${label}.current_install`, reason: "missing_local_private_beta_install" });
  if (!String(manifest.future_npm_install || "").includes("after @uploadcheck/mcp is published")) errors.push({ key: `${label}.future_npm_install`, reason: "missing_future_npm_guard" });
  ...
  if (!manifest.notes?.some((note) => note.includes("workspace API key tied to plan minutes"))) errors.push({ key: `${label}.notes`, reason: "missing_credit_gated_workspace_key_note" });
  if (!manifest.notes?.some((note) => note.includes("Do not claim Product Hunt launch readiness"))) errors.push({ key: `${label}.notes`, reason: "missing_no_launch_rule" });
}
