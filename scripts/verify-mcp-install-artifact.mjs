...
if (agentManifest.distribution?.status !== "public_github_mcp_not_npm_self_serve") {
  errors.push({ key: "agent-manifest.distribution.status", reason: "missing_public_github_status" });
}
if (agentManifest.distribution?.current_install !== "public_github_clone_or_local_checkout") {
  errors.push({ key: "agent-manifest.distribution.current_install", reason: "missing_public_github_install" });
}
...
for (const [label, manifest] of [["package", packageManifest], ["public", publicManifest]]) {
  ...
  if (manifest.distribution_status !== "public_github_mcp_not_npm_self_serve") errors.push({ key: `${label}.distribution_status`, reason: "missing_public_github_status" });
  if (manifest.current_install !== "public_github_clone_or_local_checkout") errors.push({ key: `${label}.current_install`, reason: "missing_public_github_install" });
  ...
  if (!manifest.notes?.some((note) => note.includes("Do not claim npm install readiness"))) errors.push({ key: `${label}.notes`, reason: "missing_no_npm_rule" });
}
