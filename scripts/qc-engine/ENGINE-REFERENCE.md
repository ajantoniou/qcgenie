...
2. The scripts live in `scripts/` next to this file. Run `python3 scripts/run_gate.py VIDEO --checks canvas_fill,loop_freeze,repeat_fatigue,speaker_visual_binding,static_head_dominance,literal_subject_match,first_three_seconds,end_screen_tease,rehook_cadence,contact_sheet_evidence,text_crop_jitter,thumbnail_text_readability,hallucinated_plate_text,clean_segment_source_scrub,asset_triage_reuse_manifest,chunk_sidecar_failures,script_faithfulness,sentence_boundary,dead_air,cheap_broll,text_contrast,text_safe_area,garble,twins,narration_match,omni_watch [--lang eng] [--manifest storybook.json] [--transcript transcript.txt] [--expected-script locked-script.txt] [--sidecar-dir _dialogue-chunks] [--fast]` for one verdict. For Shorts, add `shorts_format`, opening/footer, crop/jitter, and transcript-side dialogue checks explicitly: `--checks canvas_fill,shorts_format,opening_footer_text_presence,first_three_seconds,end_screen_tease,text_contrast,text_safe_area,text_crop_jitter,repeat_fatigue,sentence_boundary,dialogue_in_music_short,dead_air,garble`.
   ...
   28. `omni_watch`: multimodal pass (Qwen3-Omni via DashScope; Anthropic frame-only fallback). Grounded on the real transcript + post-filters so it never invents narration. Supplements, never sole gate.
   29. `shorts_format`: specialized Shorts check for exact 1080x1920, 50-60s duration, no gutters, opening/footer text cards, safe-area text, and optional no-dialogue verification.
