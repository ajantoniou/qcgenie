...
Checks: canvas_fill + loop_freeze + repeat_fatigue + spoken_leaks + dead_air + cheap_broll + text_contrast + text_safe_area + garble (deterministic-ish), twins + narration_match + omni_watch
...
Usage:
  run_gate.py VIDEO [--checks canvas_fill,loop_freeze,repeat_fatigue,spoken_leaks,dead_air,cheap_broll,text_contrast,text_safe_area,garble,twins,narration_match,omni_watch]
              [--lang eng] [--out DIR] [--manifest storybook.json] [--transcript transcript.txt] [--fast]
Exit 0 only if every RUN check PASSES.
"""
import sys, os, json, subprocess, argparse, time

HERE=os.path.dirname(os.path.abspath(__file__))
ALL=["canvas_fill","loop_freeze","repeat_fatigue","spoken_leaks","dead_air","cheap_broll","text_contrast","text_safe_area","garble","twins","narration_match","omni_watch","shorts_format"]
DEFAULT=["canvas_fill","loop_freeze","repeat_fatigue","spoken_leaks","dead_air","cheap_broll","text_contrast","text_safe_area","garble","twins","narration_match","omni_watch"]
SCRIPT={c:f"check_{c}.py" for c in ALL}; SCRIPT["omni_watch"]="omni_watch.py"

def run(check,video,lang,outdir,fast,manifest=None,transcript=None):
...
def main():
    ...
    ap.add_argument("--lang",default="eng"); ap.add_argument("--out",default=None); ap.add_argument("--manifest",default=None); ap.add_argument("--transcript",default=None); ap.add_argument("--fast",action="store_true")
    a=ap.parse_args()
    ...
    for c in checks:
        ...
        results[c]=run(c,a.video,a.lang,outdir,a.fast,a.manifest,a.transcript)
