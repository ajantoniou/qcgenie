...
Call `qc_get_launch_status` before launch-sensitive production workflows when an agent needs the live Product Hunt go/no-go state, remaining blockers, and operator commands. Call `qc_get_launch_handoff` when the agent needs blocker-specific required actions, proof commands, and the no-launch rule. Call `qc_get_launch_doctor` when the agent needs the blocker fix plan. These launch endpoints are public; the other QC tools still require an API key.
